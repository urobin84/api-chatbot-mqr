...
# save model
with open("data/model/model_chatbot.pkl", "wb") as model_file:
...
def chat_msg():
    args = request.args
    message = args.get("message", default="hi")
    res, tag = bot_response(message, pipeline, jp)
    return {"tag": tag, "response_message": res}, 200
